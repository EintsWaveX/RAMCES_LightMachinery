...
| `test_rev046.py` | app, DB | **The rev0.4.6 gate.** MTBF/MTTR recomputed independently from `riwayat_kondisi` and compared; the due list, the Kalibrasi tab gate and the card badge asserted to name the same assets; and stock opname driven open → count → post, including the case that matters — a movement landing WHILE the count is open, which must be measured against the CURRENT balance. Cleans up after itself |
...
```
routes                95      (88 + 6 opname + kalibrasi/jatuh-tempo, rev0.4.6)
openapi paths         66
shadow pairs           0
require_role          47 guards
broadcasts            40
