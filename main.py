...
def get_db_connection():
    try:
        ...
        print(f"CRITICAL ERROR DB: {str(e)}")
...
def register_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    """
    if db.query(models.Pengguna).count() > 0:
        raise HTTPException(
            status_code=403, detail="Registrasi publik ditutup. Hubungi admin."
        )

    if db.query(models.Pengguna).filter_by(username=user.username).first():
    ...
    db.commit()
    return {
        "message": f"User {user.username} berhasil didaftarkan sebagai {user.role}."
    }
    ...
    if not user:
        ...
        if not form_data.assigned_region:
            if form_data.role == "SUPER_ADMIN":
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Region wajib diisi untuk pendaftaran pertama.",
                )
...
def create_user(
    ...
    current_user: models.Pengguna = Depends(
        require_role(["SUPER_ADMIN", "ADMIN_DAOP"])
    ),
):
    """
    ...
    if current_user.role == "ADMIN_DAOP":
        if user_data.role != "TEKNISI":
            raise HTTPException(
                status_code=403, detail="ADMIN_DAOP hanya bisa membuat akun TEKNISI."
            )
    ...
    db.commit()
    return {
        "message": f"User {user_data.username} berhasil dibuat sebagai {user_data.role}."
    }
...
def get_all_users(
    ...
    if current_user.role == "ADMIN_DAOP":
        query = query.filter(
            models.Pengguna.assigned_region == current_user.assigned_region,
            models.Pengguna.role.in_(["ADMIN_DAOP", "TEKNISI"]),
        )
    ...
    return [
        ...
        for u in query.order_by(models.Pengguna.role, models.Pengguna.username).all()
    ]
...
def update_user(
    ...
    target = db.query(models.Pengguna).filter_by(id=user_id).first()
    if not target:
    ...
    if current_user.role == "ADMIN_DAOP":
        ...
        if target.role not in ["ADMIN_DAOP", "TEKNISI"]:
            raise HTTPException(
                status_code=403, detail="Tidak bisa mengedit SUPER_ADMIN."
            )
    ...
    if current_user.id == target.id:
        raise HTTPException(
            status_code=400,
            detail="Tidak bisa mengedit akun sendiri di sini. Gunakan fitur hapus akun.",
        )
...
def delete_user(
    ...
    if current_user.id == target.id:
        raise HTTPException(
            status_code=400,
            detail="Gunakan fitur 'Hapus Akun' untuk menghapus akun sendiri.",
        )

    if current_user.role == "ADMIN_DAOP":
        if target.assigned_region != current_user.assigned_region:
            raise HTTPException(
                status_code=403, detail="Hanya bisa menghapus user di region Anda."
            )
        if target.role not in ["ADMIN_DAOP", "TEKNISI"]:
            raise HTTPException(
                status_code=403, detail="Tidak bisa menghapus SUPER_ADMIN."
            )
...
def update_master_alat(
    kode: str, data: schemas.MasterAlatCreate, db: Session = Depends(get_db)
):
...
def create_master_lokasi(data: schemas.LokasiCreate, db: Session = Depends(get_db)):
    ...
    if data.tipe_lokasi not in valid_tipes:
        raise HTTPException(
            status_code=400, detail=f"Tipe lokasi tidak valid. Gunakan: {valid_tipes}"
        )
...
def update_master_lokasi(
    kode: str, data: schemas.LokasiCreate, db: Session = Depends(get_db)
):
...
def get_upt_by_lokasi(kode_lokasi: str, db: Session = Depends(get_db)):
    ...
    return (
        ...
        .filter_by(tipe_lokasi="UPT", parent_kode=kode_lokasi, aktif=True)
        .all()
    )
...
def get_all_aset(
    ...
    asets = (
        ...
        .options(joinedload(models.Aset.id_aset), joinedload(models.Aset.id_lokasi))
        .filter_by(status_terakhir="afkir")
        .all()
    )
...
def get_history_summary(
    ...
    for a in asets:
        ...
        if len(all_mutasi) >= 2:
            delta_secs = int(
                (all_mutasi[-1].created_at - all_mutasi[-2].created_at).total_seconds()
            )
        ...
        results.append(
            {
                ...
                # Repair summary
                "repair": {
                    "latest_date": latest_repair.created_at.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if latest_repair
                    else None,
                    ...
                    "latest_kondisi": latest_repair.status_baru
                    if latest_repair
                    else a.status_kondisi,
                },
                # Mutation summary
                "mutasi": {
                    "count": len(all_mutasi),
                    "latest_date": latest_mutasi.created_at.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if latest_mutasi
                    else None,
                    "latest_lokasi_tuju": latest_mutasi.lokasi_tuju.nama_lokasi
                    if (latest_mutasi and latest_mutasi.lokasi_tuju)
                    else None,
                    "latest_oleh": latest_mutasi.dilakukan_oleh
                    if latest_mutasi
                    else None,
                    "latest_alasan": latest_mutasi.alasan if latest_mutasi else None,
                    "delta": _format_delta(delta_secs)
                    if delta_secs is not None
                    else None,
                    "sudah_kembali": sudah_kembali,
                    "original_lokasi": a.original_lokasi_ref.nama_lokasi
                    if a.original_lokasi_ref
                    else original_kode,
                }
                if all_mutasi
                else None,
...
async def submit_mutasi(
    ...
    if lokasi_asal == mutasi.kode_lokasi_tuju:
        raise HTTPException(
            status_code=400, detail="Lokasi tujuan tidak boleh sama dengan lokasi asal."
        )

    # ADMIN_DAOP can only move assets that currently live in their region
    if (
        current_user.role == "ADMIN_DAOP"
        and lokasi_asal != current_user.assigned_region
    ):
        raise HTTPException(
            status_code=403,
    ...
    lokasi_tuju = (
        db.query(models.Lokasi)
        .filter_by(kode_lokasi=mutasi.kode_lokasi_tuju, aktif=True)
        .first()
    )
    if not lokasi_tuju:
        raise HTTPException(
            status_code=404, detail="Lokasi tujuan tidak ditemukan atau tidak aktif."
        )
...
def get_all_mutasi(
    ...
    """
    query = db.query(models.RiwayatMutasi).order_by(
        models.RiwayatMutasi.created_at.desc()
    )
        ...
        results.append(
            {
                ...
                "alat": m.aset_ref.alat_ref.nama
                if (m.aset_ref and m.aset_ref.alat_ref)
                else "—",
                "lokasi_asal": m.lokasi_asal.nama_lokasi
                if m.lokasi_asal
                else m.kode_lokasi_asal,
                "lokasi_tuju": m.lokasi_tuju.nama_lokasi
                if m.lokasi_tuju
                else m.kode_lokasi_tuju,
...
def get_public_aset(uid: str, db: Session = Depends(get_db)):
    ...
    if not aset:
        raise HTTPException(
            status_code=404, detail="Aset tidak ditemukan atau sudah di-afkir."
        )
    ...
    return {
        ...
        "unit_peruntukan": aset.unit_peruntukan,
        "tahun_pembelian": aset.tahun_pembelian,
        "last_updated": latest_riwayat.created_at.strftime("%Y-%m-%d %H:%M:%S")
        if latest_riwayat
        else None,
        ...
        "last_mutasi": {
            "dari": latest_mutasi.lokasi_asal.nama_lokasi
            if (latest_mutasi and latest_mutasi.lokasi_asal)
            else None,
            "ke": latest_mutasi.lokasi_tuju.nama_lokasi
            if (latest_mutasi and latest_mutasi.lokasi_tuju)
            else None,
            "waktu": latest_mutasi.created_at.strftime("%Y-%m-%d %H:%M:%S")
            if latest_mutasi
            else None,
        }
        if latest_mutasi
